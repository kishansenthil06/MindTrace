...
export default function Results() {
  const { result, saveResult, history, historyState } = useStore();
        ...
        <motion.div variants={staggerContainer} initial="hidden" animate="show" className="grid gap-4 sm:grid-cols-3">
          {[
            ...
            >
              ...
              <p className="mt-3 font-display text-2xl font-semibold">{value?.toFixed(1)}</p>
          ...
          {modalities.map(([key, value]) => {
            ...
            return (
              ...
              >
                ...
                <small className="mt-1 block truncate text-xs text-slate-500">{value.label}</small>
                <div className="mt-4 flex items-center justify-between text-[11px] text-slate-500">
                  <span>Influence</span>
                  <span className="font-mono text-slate-300">{value.contribution}%</span>
                </div>
                <div className="mt-2 h-1 overflow-hidden rounded-full bg-[#1C1F2B]">
                  <motion.div
                    className="h-full rounded-full bg-cyan-400/80"
                    initial={{ width: 0 }}
                    animate={{ width: `${Math.min(value.contribution, 100)}%` }}
